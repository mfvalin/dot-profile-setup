 ...
 */
static int calc_crc(p, flen, fseed, stride)
	unsigned char *p;
...

{
	...
	register uint32_t i;			/* index into data */
...
static int f_calc_crc(p, f_flen, f_fseed, f_stride)
	unsigned char *p;
...
static int f_calc_crc_(p, f_flen, f_fseed, f_stride)
	unsigned char *p;
        int *f_flen, *f_fseed, *f_stride;
{
        int stride = *f_stride;
...
static int f_calc_crc__(p, f_flen, f_fseed, f_stride)
	unsigned char *p;
        int *f_flen, *f_fseed, *f_stride;
{
        int stride = *f_stride;
...
char * product_id_tag="$Id: r.crc.c 76 2014-02-07 19:18:22Z armnlib $";
